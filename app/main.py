...
logger = logging.getLogger("incubator_v3")
...
def startup() -> None:
    ...
    try:
        ...
        logger.info("Startup route table contains %d routes", len(route_paths))
        logger.info("Root route registered: %s", "/" in route_paths)
    finally:
        db.close()
...
def dashboard(request: Request, db: Session = Depends(get_db), session_token: str | None = Cookie(default=None, alias=settings.session_cookie_name)):
    ...
    try:
        return templates.TemplateResponse("index.html", {"request": request, "settings": get_settings(db), "version": settings.app_version})
    except Exception:
...
def api_version() -> dict:
    return {"version": settings.app_version, "scheme": "M.mm"}
